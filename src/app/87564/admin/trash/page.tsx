  ...
  return (
    <AdminShell title="Trash & Archive Management" subtitle="Manage soft-deleted entries. Restore items to frontend immediately or permanently delete them (Super Admins only).">
      <div className="space-y-6">
        ...
        {/* Tab Headers */}
        <div className="border-b border-[#F5C2C2] bg-[#FFFDF8] p-2 rounded-t-xl flex flex-wrap gap-1">
            ...
            <button
              ...
              className={`rounded-lg px-4 py-2.5 text-xs font-black uppercase tracking-wider transition-all duration-200 ${
                activeTab === tab
                  ...
                  : "text-[#5F6752] hover:bg-[#F8F7F3] hover:text-[#2B2B2B]"
              }`}
            >
              {getTabLabel(tab)}
            </button>
        ...
        {/* Trash Content Table */}
        <div className="overflow-hidden rounded-b-xl border border-t-0 border-[#F5C2C2] bg-[#FFFDF8] shadow-sm">
              ...
              <tbody className="divide-y divide-[#E9E1D5]">
                {loading ? (
                  ...
                  items.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-[#FAF9F6]/70 transition-colors">
                        {(activeTab === "products" || activeTab === "categories" || activeTab === "subcategories" || activeTab === "brands") && (
                          <td className="px-5 py-3">
                            {thumb ? (
                              <img src={thumb} alt={title} className="h-10 w-10 rounded-md object-cover border border-[#E5DED2]" />
                            ) : (
                              <div className="flex h-10 w-10 items-center justify-center rounded-md bg-[#F8F7F3] text-[#9A9387]">
                        ...
                        <td className="px-5 py-4">
                          <div className="flex gap-2">
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 rounded-lg border border-[#F5C2C2] bg-[#FFFDF8] px-2.5 py-1.5 text-xs font-bold text-[#C62828] shadow-sm hover:bg-[#FAF9F6] disabled:opacity-50"
                            >
                              <RotateCcw className="h-3.5 w-3.5 text-[#D32F2F]" /> Restore
                            </button>
                            <button
                              disabled={!isSuperAdmin || processingId !== null}
                              onClick={() => handlePermanentDelete(item.id)}
                              className="inline-flex items-center gap-1.5 rounded-lg border border-[#E6C8B4] bg-[#FFFDF8] px-2.5 py-1.5 text-xs font-bold text-[#D32F2F] shadow-sm hover:bg-[#F8F7F3] disabled:opacity-40"
