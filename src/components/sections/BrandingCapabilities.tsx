...
const CAPABILITIES: Capability[] = [
  {
    icon: <Cpu className="w-6 h-6 text-[#D32F2F]" />,
  ...
  {
    icon: <Layers className="w-6 h-6 text-[#EF5350]" />,
  ...
  {
    icon: <Scissors className="w-6 h-6 text-[#D32F2F]" />,
  ...
  {
    icon: <Sparkles className="w-6 h-6 text-[#EF5350]" />,
...
export function BrandingCapabilities() {
  return (
    <section className="py-24 bg-[#2B2B2B] border-t border-white/5 relative overflow-hidden text-white">
      {/* Background Lighting Effects */}
      <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-[#D32F2F]/10 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-[#EF5350]/5 rounded-full blur-[140px] pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Section Heading */}
        <div className="text-center max-w-3xl mx-auto mb-20">
          <span className="px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs font-extrabold tracking-widest uppercase mb-4 inline-block text-[#EF5350]">
            Craftsmanship & Technology
          </span>
          <h2 className="text-3xl md:text-5xl font-black tracking-tight text-white mb-6">
            Enterprise <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#EF5350] via-[#e8c68a] to-[#EF5350]">Branding Capabilities</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {CAPABILITIES.map((cap, idx) => (
            <motion.div
              ...
              whileHover={{ y: -6, borderColor: "rgba(201, 92, 107, 0.3)" }}
              className="bg-white/5 border border-white/10 rounded-2xl p-8 flex flex-col justify-between hover:shadow-[0_12px_40px_rgba(200,163,106,0.06)] transition-all duration-300 relative group text-left"
            >
              <div>
                {/* Icon box */}
                <div className="mb-6 bg-white/10 w-12 h-12 rounded-xl flex items-center justify-center border border-white/10 shadow-inner group-hover:scale-105 transition-transform duration-300">
                  {cap.icon}
                </div>

                <div className="space-y-2 mb-6">
                  <span className="text-[10px] font-extrabold uppercase tracking-widest text-[#EF5350]">{cap.subtitle}</span>
                  <h3 className="text-lg font-bold text-white tracking-tight leading-tight group-hover:text-[#EF5350] transition-colors">
                    {cap.title}
                  </h3>
              ...
              <div className="pt-5 border-t border-white/5 space-y-2 mt-auto">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {cap.materials.map((mat) => (
                    ...
                    >
                      <CheckCircle2 className="w-2.5 h-2.5 text-[#D32F2F] flex-shrink-0" />
                      {mat}
                    </span>
                  ))}
                </div>
